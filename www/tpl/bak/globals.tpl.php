...
<? if(isAdmin() && $showheader){ ?>
		...
		<td width="175">  
			<h1>
				<a href="<?=$pre.$class?>"><?=T('adm_globs');?></a> 
...
<?php switch($do){
	...
	<tr>
		<thead>
			<td width="100"><b><font color="3e3e3e"><?=T('adm_name');?></font></b></td>
			<td width="580"><b><font color="3e3e3e"><?=T('adm_value');?></font></b></td>
	...
	// Add\Edit  item
	case 'item' : ?>
		<form id="globalsform" method="POST" action="<?=$pre.$class?>/save">
		<input type="hidden" name="id" id="id" value="<?=$id;?>">
			...
			<?php   unset($data[0]); //inspect($data);  inspect($fields);?>				
				...
				<tr>
					<td align="right"><?=T('adm_value');?>:</td>
					<td><input type=text value="<?=@$data['value'];?>" name='form[value]' id='value'></td>
				</tr>	
				<tr>
					<td></td>
					<td align="left">
						<a onclick="$('globalsform').submit()" href="javascript:void(0)">
