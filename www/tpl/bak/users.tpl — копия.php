...
switch($do){ 	
	...
	<section id="login" class="login  center">
		<form id="loginForm" method="POST" action="<?=BASE_PATH;?>login" onsubmit="return false;">
			<div class="regtable center">
				...
				<input name="form[pass]" id="login_pass" type="password" value="" onfocus="empt(this,'<?=T('pass');?>','passfield')" onblur="chk(this,'<?=T('pass');?>','passfield')">
				...
				<a href="javascript:void(0)" class="switch pass" data-section="restore"><?=T('forgot pass');?></a>
				
				<!--<p class="pass" style="color:#828282">
					<?php echo T("Don't have an account?");?> 
					<a class="pass" href="<?=BASE_PATH;?>register" data-section="register" target="#formModal"><?php echo T("join now");?></a>
				</p>-->
	...
	<section class="hide center" id="restore">
		<form id="regiForm" class="pad5" method="POST" action="<?=BASE_PATH;?>recover" onsubmit="return false;">
			<div class="regtable">
				<h2><?=T('recover');?></h2>
				<div class="sendmaildiv">		
					<?=T('enteremail');?></div>
				<input name="form[mail]" id="restore_mail" style="margin-bottom:5px;margin-top:10px;"><br>
				<center><a href="javascript:void(0)" onclick="$('#regiForm').submit()">
					<div class="bigGreenButton" style="margin-left:5px !important;"><?=T('submit');?></div>
				</a><br>
				<a href="javascript:void(0)" data-section="login" class="switch">&larr; <?php echo T('go back');?></a>
				</center>
				<div class="errors">
						<p><?=T('wrongMail');?></p>
				</div>
				<div id="sentmail" style="display:none"><table width="100%"><tr><td class="sm"><?=T('sentMail');?></td></tr></table>
				</div>
			</div>
		</form>
		<script>
			$('#regiForm').on( "submit", function( event ) {			
			event.preventDefault();		
			var form = $(this);
			if($("#restore_mail").val() == '') {
				$('.errors p').fadeIn();
			...
			$.ajax({
				...
				success: function(res, status) {
				   if (res == 'ok') {
					$("#restore").html($("#sentmail").html());
					setTimeout("location.href='<?=BASE_PATH;?>';",3000);
					} else {
						$('.errors p').fadeIn();
					}
				}
...
<? break;
	...
	case 'register': ?> 	
	<form id="regiForm" method="POST" action="<?=BASE_PATH;?>register" onsubmit="return false;">
		<div class="regtable center">
			...
			<label for="login_pass" id="passfield" class="passfield"><?=T('pass');?></label>	
			<input name="form[pass]" onfocus="empt(this,'<?=T('pass');?>','passfield')" onblur="chk(this,'<?=T('pass');?>','passfield')" id="regPass" type="password" value="">
...
<? break;
	...
	$my = (bool)($id == $_SESSION['user']['id']);  //inspect($my);
/*	if($data['confirmed'] < 2 && $my){ $do = 'education'; $first = true; 
	 ?>
	...
	<table cellpadding=0 cellspacing=0>
		...
		<tr bgcolor="#eeeeee">
			<td colspan=2>
				<a href="<?=BASE_PATH;?>users/upphoto"><?=T('upload photo');?></a> | 
				<a href="<?=BASE_PATH;?>users/settings"><?=T('my settings');?></a> | 
				<a href="<?=BASE_PATH;?>users/education"><?=T('my education');?></a> | 
		...
		<tr>
			<td class="leftmenu"  width="200px" valign="top" style="padding-bottom:0px;">
				...
				<img src="<?=$url;?>" align="right" width="201" style="border-right:1px gray solid !important;margin-left:2px">
				<? if($my){ ?></a> <? } ?>
				<? 	if($id==1){
						$uni = "ISMA";
			...
			<td  valign="top" class="wraptop" rowspan=2>
			 <b class="name"><?=$data['name'].' '.$data['surname'];?></b> 
				<? if($my){ // inspect($data); ?>
				...
				<div id="light" class="white_content">
				 ...
				 <? $contacts = array_flip(split(',',$_SESSION['user']['contacts'])); //inspect($_SESSION['user']);
			...
			<td class="leftmenu"  width="200px" valign="top">
				...
				<a href="javascript:void(0)" onclick="showHide('aboutme')" class="b"><?=T('About');?></a>
					<div id="aboutme">			
					 ...
					 <a href="<?=BASE_PATH.'contacts'.($my?'':'/'.$id);?>"  class="b"><?=T('contacts');?></a>
				    <? //if($my) { ?>
					<div class="friends">
					<? //inspect($data['friends']);
						$i = 0;
						foreach ($data['friends'] as $row){ ?>
							<div>	
								...
								<? }else{ ?>
								<img src="<?=PUB;?>img/defaultavatarsml.png">
								<? } ?><p><?=$row['name'];?></p><p><?=$row['surname'];?></p></a>
							</div>
							<? $i++; if($i==3) echo "</div><div class='friends'>";// "</tr><tr>";
						}
					?>
					</div>
					<? //} ?>
	...
	/*	drawSelect('cities','<?=$data['country'];?>','country_id','city','drawUnies','<?=$data['city_id'];?>');
		//setSelVal('<?=$data['city_id'];?>','citysel');
		drawSelect('universities','<?=$data['city_id'];?>','city_id','uni','drawFacs','<?=$data['uni_id'];?>');
	//	setSelVal('<?=$data['uni_id'];?>','unisel');
		drawSelect('faculties','<?=$data['uni_id'];?>','uni_id','faculty','drawSpec','<?=$data['faculty_id'];?>');
		drawSelect('specialization','<?=$data['faculty_id'];?>','faculty_id','specialization','','<?=$data['specialization_id'];?>');
	//	setSelVal('<?=$data['faculty_id'];?>','facultysel');
	//	setSelVal('<?=$data['specialization_id'];?>','specializationsel');*/
	</script>
...
<? }
break;
	case 'contacts': //inspect($data);?>
	<div class="wrapp" style="width:687px">
	<h4 class="subhd" style="width:682px"><?=T('contacts');?></h4>
			<span class="wptxt">
	...
	<div>
	<center class="halepa">
		<img src="<?=PUB;?>www/img/halepa.jpg"><br>
		<h2><?=T('halepa');?></h2><br>
