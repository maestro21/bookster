<div class="wrapdiv"><? 
	if(isAdmin() && $showheader){ ?>
		...
		<td width="175">
			<h1>
			<a href="<?=$pre.$class?>/items"><?=T('adm_langs');?></a>
...
<?php switch($do){ 
	...
	<table class="admSearchTableMain" cellspacing="0">
		<tr>
			<thead>
				<td width="50"><b><font color="3e3e3e"><?=T('adm_abbr');?></font></b></td>
				<td width="500"><b><font color="3e3e3e"><?=T('adm_lang');?></font></b></td>
				<td width="100"><b><font color="3e3e3e"><?=T('adm_enabled');?></font></b></td>
	...
	// Add\Edit  item
	case 'item' : ?>
		<form id="langform" method="POST" action="<?=$pre.$class?>/save">
		<input type="hidden" name="id" id="id" value="<?=$id;?>">
			<table class="admAddTable">
			<?php   unset($data[0]); //inspect($data);  inspect($fields);?>
				<tr>
					<td align="right"><?=T('adm_abbr');?>:</td>
					<td align="left"><input type=text value="<?=@$data['abbr'];?>" name='form[abbr]' id='abbr'></td>
				</tr>
				<tr>
				...
				<tr>
					<td align="right"><?=T('adm_enabled');?>:</td>
					<td>
						<input type=hidden name='form[enabled]' value='1'>
							<input type=checkbox  value=1 <?=(@$data['enabled']==1?"checked":"");?> name='data[enabled]' id='enabled'></td>
				</tr>
				<tr>
					<td align="right"><?=T('adm_labels');?>:</td>
					<td>
						<textarea cols=50 rows=20 name='form[labels]' id=text><?=@$data['labels'];?></textarea>
					</td>
				</tr>
				<tr>
					<td></td>
					<td align="left">
						<a onclick="$('langform').submit()" href="javascript:void(0)">
