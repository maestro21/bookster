...
<table class="admSearchTable">
	<td width="175">
		<h1>
			<a href="<?=$pre.$class?>/items"><?=T('adm_facs');?></a>
...
<?php switch($do){ 
	...
	<tr>
		<thead>
			<td width="160"><b><font color="3e3e3e"><?=T('adm_uni');?></font></b></td>
			<td width="485"><b><font color="3e3e3e"><?=T('adm_fac');?></font></b></td>
	...
	// Add\Edit  item
	case 'item' : ?>
		<form id="facform" method="POST" action="<?=$pre.$class?>/save">
		<input type="hidden" name="id" id="id" value="<?=$id;?>">
			...
			<?php   unset($data[0]); //inspect($data);  inspect($fields);?>
				<tr>
					...
					<td>
						<select name="form[uni_id]" id="uni_id">
							<? foreach ($options['uni_id'] as $k =>$v){ ?>
								<option value='<?=$k;?>'<? if($k==@$data['uni_id']) echo ' selected="selected"';?>><?=T($v);?></option>
							<? } ?>
						</select></td>
				</tr>
				<tr>
				...
				<tr>
					<td></td>
					<td align="left">
						<a onclick="$('facform').submit()" href="javascript:void(0)">
